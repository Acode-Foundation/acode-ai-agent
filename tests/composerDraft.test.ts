...
import {
  ...
  mentionQueryAt,
  rankMentionFiles,
} from "../src/workspace/fileMentions.ts";
import {
  mimeFromName,
  modelAcceptsImages,
  normalizeImageMime,
  toPiImages,
} from "../src/platform/promptImages.ts";
...
test("packages picked files for the model while keeping a compact file chip", () => {
  ...
  expect(restored.files).toMatchObject([
    { name: 'outside".ts', content: "const end = `]]>`;", encoding: "text" },
  ]);
});
...
test("builds Pi paste markers and expands them for the model", () => {
  ...
  expect(
    expandPasteMarkers("see [paste #1 +12 lines] please", [
      {
        id: "paste-1",
        name: "paste #1 +12 lines",
        content: lines,
        encoding: "text",
        kind: "paste",
        pasteId: 1,
...
test("builds Pi image blocks without a data-url prefix", () => {
  expect(
    toPiImages([{ type: "image", data: "data:image/png;base64,abcd", mimeType: "image/png" }]),
  ).toEqual([{ type: "image", data: "abcd", mimeType: "image/png" }]);
  ...
  expect(
    modelAcceptsImages({ input: ["text"], inputModalitiesKnown: false } as never),
  ).toBeUndefined();
});
