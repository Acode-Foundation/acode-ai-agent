...
test("Codex browser login exchanges a state-validated callback with matching PKCE", async () => {
  ...
  const fetchMock = vi.fn(async (input: string, init?: RequestInit) => {
    if (input.includes("/codex/models?"))
      return Response.json({ models: [{ slug: "gpt-5.6-terra", visibility: "list" }] });
...
test.each(["wrong-state", "missing-state", "missing-code", "denied", "wrong-url", "raw-code"])(
  "Codex rejects %s before exchanging tokens",
  async (scenario) => {
    let state = "";
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    await expect(
      portableCodexOAuth.login({
        notify(event) {
          if (event.type === "auth_url") state = new URL(event.url).searchParams.get("state")!;
        },
        async prompt(prompt) {
          if (prompt.type === "select") return "browser";
