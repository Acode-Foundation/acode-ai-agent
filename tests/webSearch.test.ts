...
import {
  ...
  parseScrapedResults,
  searchWithWebView,
} from "../src/tools/web/webviewSearch.ts";
...
test("rewrites GitHub blob URLs to raw file contents", () => {
  ...
  expect(rewriteGithubBlob(new URL("https://example.com/blob/main/x"))).toBe(
...
test("presents web tools in the work log", () => {
  expect(
    presentTool(
      "web_search",
      { query: "zod 4 migrate" },
      "Web search via openai · “zod 4 migrate”\n\nSources",
    ),
  ).toEqual({
    kind: "web",
...
test("falls back from native OpenAI search to the device WebView", async () => {
  ...
  const restore = installWebView([
    { title: "Docs", url: "https://docs.example/guide", snippet: "Official guide" },
  ]);
  try {
...
test("skips native search when the model provider has none", async () => {
  ...
  try {
    ...
    expect(response.provider).toBe("webview");
    expect(calls.some((url) => url.includes("api.openai.com") || url.includes("api.x.ai"))).toBe(
      false,
    );
  } finally {
    globalThis.fetch = previous;
...
test("unwraps DDG redirect links and drops search-engine junk", () => {
  expect(
    cleanUrl("https://duckduckgo.com/l/?uddg=https%3A%2F%2Fdocs.example.com%2Fapi&utm_source=ddg"),
  ).toBe("https://docs.example.com/api");
  expect(cleanUrl("https://www.bing.com/search?q=widgets")).toBe("");
  expect(cleanUrl("https://docs.python.org/3/library/os.html?utm_medium=share")).toBe(
...
test("parses device WebView scrape payloads", () => {
  ...
  ).toEqual([{ title: "Docs", url: "https://docs.example/x", snippet: "Guide" }]);
});
