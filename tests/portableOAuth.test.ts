...
test("Kimi subscription refresh derives a bearer header", async () => {
  ...
  try {
    const credential = await portableKimiOAuth.refresh({
      type: "oauth",
      access: "old",
    ...
    expect(await portableKimiOAuth.toAuth(credential)).toEqual({
      headers: { Authorization: "Bearer kimi-access" },
    });
  } finally {
    restore();
  }
...
test("advertises every portable subscription sign-in", () => {
  const labels = Object.fromEntries(
    PROVIDERS.filter((provider) => provider.subscriptionLabel).map((provider) => [
      provider.id,
...
test("filters pi's complete Copilot catalog to models available for the signed-in account", async () => {
  ...
  const modelIds = (await registry.refreshModelAvailability("github-copilot")).map(
    (model) => model.id,
  );
