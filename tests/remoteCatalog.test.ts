...
test("merges, validates, and restores Pi's cached provider catalog", async () => {
  ...
  expect(first.provider.getModels().find((model) => model.id === existing.id)?.name).toBe(
    "Updated upstream",
  );
  expect(first.provider.getModels().find((model) => model.id === added.id)?.input).toEqual([
    "text",
    "image",
  ]);
...
test("revalidates stale catalogs with ETag and keeps cached models on 304", async () => {
  ...
  await store.write("openrouter", {
    models: [{ ...existing, name: "Cached name" }],
    checkedAt: 1,
    etag: '"old"',
  });
  ...
  expect(catalog.provider.getModels().find((model) => model.id === existing.id)?.name).toBe(
    "Cached name",
  );
