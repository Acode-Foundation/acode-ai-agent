...
test("resizes screenshots and returns Pi coordinate mapping hints", async () => {
  ...
  const output = await browserReadImageProcessor(new Uint8Array([1]), "image/png", {
  ...
  expect(output).toEqual({
    ...
    mimeType: "image/png",
    hints: [
      "[Image: original 4000x2000, displayed at 2000x1000. Multiply coordinates by 2 to map to the original image.]",
    ],
  });
...
test("converts BMP reads to PNG even when automatic resizing is disabled", async () => {
  ...
  const output = await browserReadImageProcessor(new Uint8Array([1]), "image/bmp", {
