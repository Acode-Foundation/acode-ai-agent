...
function assistant(
  content: AgentMessage extends { role: "assistant"; content: infer C } ? C : never,
  timestamp = 2,
): AgentMessage {
  return {
    role: "assistant",
  ...
  const turns = buildTurns([
    ...
    ),
    {
      ...
      content: [
        {
          type: "text",
          text: "[Showing lines 1-2000 of 8432. Use offset=2001 to continue.]\n\nconst x = 1;",
        },
      ],
      isError: false,
      timestamp: 3,
...
test("labels common tools", () => {
  expect(presentTool("grep", { query: "TODO" })).toEqual({
    kind: "search",
  ...
  expect(presentTool("bash", { command: "npm test" })).toEqual({
    kind: "terminal",
    label: "Ran command",
    detail: "npm test",
  });
...
test("while live, shows every running tool and groups the rest", () => {
  const split = splitWorkBurst(
    [tool("t1"), tool("t2", "running"), tool("t3", "error"), tool("t4", "running")],
    true,
  );
...
test("parses grep and glob output into navigable file results", () => {
  expect(
    parseToolFileResults("grep", "src/ui/App.tsx:42: return <App />;\nsrc/main.ts:8: mountApp();"),
  ).toEqual([
