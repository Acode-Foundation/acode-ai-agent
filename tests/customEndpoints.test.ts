...
test("normalizes OpenAI-compatible base URLs and rejects unsafe ones", () => {
	...
	expect(normalizeOpenAIBaseUrl("http://192.168.1.10:11434/v1/")).toBe("http://192.168.1.10:11434/v1");
	expect(normalizeOpenAIBaseUrl("https://proxy.example.com/openai/v1")).toBe("https://proxy.example.com/openai/v1");
	expect(normalizeOpenAIBaseUrl("http://host:8080/v1/chat/completions")).toBe("http://host:8080/v1");
...
test("parses OpenAI, Ollama, and string-list model catalogs", () => {
	expect(parseOpenAIModelIds({ data: [{ id: "llama3.1:8b" }, { id: "qwen2.5-coder:7b" }] })).toEqual([
		"llama3.1:8b",
		"qwen2.5-coder:7b",
	]);
...
test("drops invalid persisted endpoints and keeps a safe catalog", () => {
	expect(sanitizeCustomEndpoints([
...
test("finalize and upsert preserve an existing id", () => {
	const first = finalizeCustomEndpoint({ name: "LM Studio", baseUrl: "http://10.0.0.4:1234/v1", models: "local-model" }, []);
	expect(first.id).toBe("lm-studio");
	const updated = finalizeCustomEndpoint({ ...first, name: "Home GPU", models: "local-model\nother" }, [first]);
	expect(updated.id).toBe("lm-studio");
	expect(updated.name).toBe("Home GPU");
	expect(updated.models).toEqual(["local-model", "other"]);
	expect(upsertCustomEndpoint([first], updated).map((endpoint) => endpoint.id)).toEqual(["lm-studio"]);
});
...
test("local presets use LAN IPs and llama.cpp / vLLM ports", () => {
	expect(LOCAL_ENDPOINT_PRESETS.map((preset) => preset.name)).toEqual(["llama.cpp", "vLLM", "LM Studio", "Ollama"]);
...
test("registry lists and resolves a custom llama.cpp endpoint", () => {
	...
	expect(registry.descriptors().find((item) => item.id === "llama-cpp")).toMatchObject({ id: "llama-cpp", name: "llama.cpp", hint: "http://192.168.1.10:8080/v1" });
...
test("probeOpenAIModels reads /models with an optional bearer token", async () => {
	...
	try {
		await expect(probeOpenAIModels("http://192.168.1.10:11434", "sk-test")).resolves.toEqual(["llama3.1:8b"]);
		expect(requests).toEqual([{ url: "http://192.168.1.10:11434/v1/models", auth: "Bearer sk-test" }]);
	} finally {
