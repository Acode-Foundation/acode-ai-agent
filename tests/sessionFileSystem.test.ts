...
afterEach(async () => { vi.unstubAllGlobals(); for (const fn of cleanups.splice(0)) await fn(); });
async function setup() { const fixture = await sessionFileSystemFixture(); cleanups.push(fixture.cleanup); return fixture; }
...
test("appends UTF-8 bytes, overwrites shorter content, and replaces a sibling destination", async () => {
	...
	getOrThrow(await adapter.appendFile("/sessions/chat.jsonl", new TextEncoder().encode("世界\n"), context));
	expect(getOrThrow(await adapter.readTextFile("/sessions/chat.jsonl", context))).toBe("hello 👋\n世界\n");
...
test("rejects paths outside its private namespace and cross-directory rename", async () => {
	const { adapter } = await setup();
	for (const path of ["../../secret", "file:///secret", "a\\b", "bad\0name"]) expect((await adapter.writeFile(path, "bad", context)).ok).toBe(false);
...
test("propagates append errors and leaves the original intact when rename fails", async () => {
	...
	const failingHost = ((path: string) => ({ ...host(path), renameTo: async () => { throw new Error("rename failed"); } })) as Acode.FS;
	const failing = new SessionFileSystem(uri, failingHost, async () => { throw new Error("disk full"); });
...
test("Cordova append seeks to byte length and rejects writer failure", async () => {
	...
	const writer = { length: 7, seek: vi.fn(), write: vi.fn(), onwriteend: undefined as (() => void) | undefined, onerror: undefined as ((event: unknown) => void) | undefined };
	vi.stubGlobal("resolveLocalFileSystemURL", (_uri: string, success: (entry: unknown) => void) => success({ createWriter: (ready: (writer: unknown) => void) => ready(writer) }));
