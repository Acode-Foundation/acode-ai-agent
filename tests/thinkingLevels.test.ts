...
test("Grok 4.6 overlay adds xhigh and is merged into the frozen catalog", () => {
  ...
  expect(thinkingLevelsFor(overlay!).map((level) => level.id)).toEqual([
    "low",
    "medium",
    "high",
    "xhigh",
...
test("models without configurable reasoning hide effort and clamp to off", () => {
  ...
  expect(
    clampThinkingLevel({ ...grok45, reasoning: false, thinkingLevelMap: undefined }, "high"),
  ).toBe("off");
});
...
test("uses pi's opt-in rules for extended OpenRouter efforts", () => {
  ...
  expect(thinkingLevelsFor(openrouterModel).map((level) => level.id)).toEqual([
    "off",
    "minimal",
    "low",
    "medium",
