...
import {
  ...
  stripCredentials,
  workspaceId,
  workspaceRelativeFromIndex,
} from "../src/workspace/pathSandbox.ts";
...
test("maps a SAF folder opened as tree::documentId without leaking the document id", () => {
  const root =
    "content://com.android.externalstorage.documents/tree/primary%3ACodes::primary:Codes";
  const sandbox = new PathSandbox(root, join);
  expect(sandbox.relative(root)).toBe("");
  expect(sandbox.relative(`${root}/src/main.ts`)).toBe("src/main.ts");
  expect(
    sandbox.relative(
      "content://com.android.externalstorage.documents/tree/primary%3ACodes::primary:Codes/lib/util.ts",
    ),
  ).toBe("lib/util.ts");
...
test("maps nested SAF folders the same way as the tree root", () => {
  const root =
    "content://com.android.externalstorage.documents/tree/primary%3ACodes::primary:Codes/app";
  const sandbox = new PathSandbox(root, join);
  expect(sandbox.relative(`${root}/src/main.ts`)).toBe("src/main.ts");
  expect(sandbox.resolve("src/main.ts").relativePath).toBe("src/main.ts");
...
test("maps Termux SAF document ids that start with an absolute unix path", () => {
  const tree =
    "content://com.termux.documents/tree/%2Fdata%2Fdata%2Fcom.termux%2Ffiles%2Fhome%2Fproj";
  const sandbox = new PathSandbox(tree, join);
  expect(sandbox.relative(`${tree}::/data/data/com.termux/files/home/proj/src/main.ts`)).toBe(
    "src/main.ts",
  );
});
...
test("maps file-index records onto workspace-relative paths", () => {
  ...
  const sandbox = new PathSandbox(root, join);
  expect(
    workspaceRelativeFromIndex({ url: `${root}::src/main.ts`, name: "main.ts" }, sandbox, "Dev"),
  ).toBe("src/main.ts");
  expect(
    workspaceRelativeFromIndex({ path: "Dev/src/main.ts", name: "main.ts" }, sandbox, "Dev"),
  ).toBe("src/main.ts");
  expect(workspaceRelativeFromIndex({ path: "Dev", name: "Dev" }, sandbox, "Dev")).toBe("");
  expect(workspaceRelativeFromIndex({ path: "src/main.ts", name: "main.ts" }, sandbox, "Dev")).toBe(
    "src/main.ts",
  );
  expect(
    workspaceRelativeFromIndex(
...
test("maps an Acode FTP folder from Url.formate (user, password, port, mode, security)", () => {
  ...
  expect(sandbox.relative(root)).toBe("");
  expect(
    sandbox.relative("ftp://alice:p%40ss@ftp.example.com:21/www?mode=passive&security=ftp"),
  ).toBe("");
  expect(sandbox.relative(file)).toBe("src/main.ts");
  expect(
    sandbox.relative(
      "ftp://alice:p%40ss@ftp.example.com:21/www/lib/util.ts?mode=passive&security=ftp",
    ),
  ).toBe("lib/util.ts");
  expect(
    sandbox.relative(
      "ftp://alice:p%40ss@ftp.example.com:21/other/src/main.ts?mode=passive&security=ftp",
    ),
  ).toBeUndefined();
  expect(sandbox.resolve("src/main.ts")).toEqual({
    relativePath: "src/main.ts",
  ...
  expect(
    workspaceRelativeFromIndex(
      { url: file, path: "www/src/main.ts", name: "main.ts" },
      sandbox,
      "www",
    ),
  ).toBe("src/main.ts");
  expect(
    sameParentUri(root, "ftp://alice:p%40ss@ftp.example.com:21/www?mode=passive&security=ftp"),
  ).toBe(true);
...
test("maps an Acode FTP folder that only has a username", () => {
  ...
  expect(
    sandbox.relative(
      "ftp://anonymous@ftp.example.com:21/public_html/css/app.css?mode=passive&security=ftp",
    ),
  ).toBe("css/app.css");
  expect(
    sandbox.relative(
      "ftp://anonymous@other.example.com:21/public_html/css/app.css?mode=passive&security=ftp",
    ),
  ).toBeUndefined();
});
...
test("maps an Acode SFTP profile URL (sftp://profile-id/path)", () => {
  ...
  expect(
    workspaceRelativeFromIndex(
      { url: file, path: "Codes/src/main.ts", name: "main.ts" },
      sandbox,
      "Codes",
    ),
  ).toBe("src/main.ts");
...
test("maps an Acode SFTP storage root created by createSftpProfileUrl", () => {
  ...
  expect(sandbox.relative("sftp://profile-k3m9x2ab/home/alice/Codes/src/main.ts")).toBe(
    "home/alice/Codes/src/main.ts",
  );
  expect(sandbox.resolve("home/alice/Codes/src/main.ts").uri).toBe(
    "sftp://profile-k3m9x2ab/home/alice/Codes/src/main.ts",
  );
});

test("does not mix Acode ftp, sftp profile, file, and SAF workspaces", () => {
  const ftp = new PathSandbox(
  ...
  expect(
    ftp.relative(
      "content://com.android.externalstorage.documents/tree/primary%3ACodes::primary:Codes/src/main.ts",
    ),
  ).toBeUndefined();
  expect(
    sftp.relative(
      "ftp://alice:secret@ftp.example.com:21/home/alice/Codes/src/main.ts?mode=passive&security=ftp",
    ),
  ).toBeUndefined();
  expect(sftp.relative("file:///home/alice/Codes/src/main.ts")).toBeUndefined();
  expect(
    file.relative(
      "ftp://alice:secret@ftp.example.com:21/storage/emulated/0/Codes/src/main.ts?mode=passive&security=ftp",
    ),
  ).toBeUndefined();
  expect(saf.relative("sftp://profile-k3m9x2ab/Codes/src/main.ts")).toBeUndefined();
  expect(
    saf.relative(
      "ftp://alice:secret@ftp.example.com:21/Codes/src/main.ts?mode=passive&security=ftp",
    ),
  ).toBeUndefined();
...
test("strips credentials from Acode FTP and legacy SFTP URLs", () => {
  ...
  const legacySftp =
    "sftp://alice:secret@git.example.com/project?password=secret&keyFile=/private/key";
  ...
  expect(workspaceId(ftp)).toBe(
    workspaceId("ftp://alice:other@ftp.example.com:21/www?mode=active&security=ftps"),
  );
  expect(workspaceId(legacySftp)).toBe(
    workspaceId("sftp://alice:other@git.example.com/project?password=other"),
  );
...
test("maps Termux SAF whether the folder was opened as a tree or as tree::docId", () => {
  ...
  expect(
    asTree.relative(`${tree}::/data/data/com.termux/files/home/other/src/main.ts`),
  ).toBeUndefined();
});
