...
test("merges built-ins, prompt templates, and Pi skill commands", () => {
  const commands = resourceSlashCommands({
    promptTemplates: [
      { name: "review", description: "Review this change", content: "Review $ARGUMENTS" },
    ],
    skills: [
      {
        name: "frontend",
        ...
        filePath: ".agents/skills/frontend/SKILL.md",
      },
    ],
  });
  expect(commands.some((command) => command.name === "model" && command.source === "action")).toBe(
    true,
  );
  expect(commands.some((command) => command.name === "tasks" && command.source === "action")).toBe(
    true,
  );
  expect(commands.some((command) => command.name === "review" && command.source === "prompt")).toBe(
    true,
  );
  expect(
    commands.some((command) => command.name === "skill:frontend" && command.source === "skill"),
  ).toBe(true);
});
...
test("command filtering ranks exact and prefix matches before description matches", () => {
  const commands = resourceSlashCommands({
    promptTemplates: [
      { name: "review", description: "Inspect the model setup", content: "Review" },
    ],
  });
...
test("loads project skills and prompts from Acode's virtual workspace", async () => {
  ...
  const directories: Record<
    string,
    Array<{ path: string; name: string; isFile: boolean; isDirectory: boolean }>
  > = {
    ".agents/skills": [
      { path: ".agents/skills/reviewer", name: "reviewer", isFile: false, isDirectory: true },
    ],
    ...
    ".pi/prompts": [
      { path: ".pi/prompts/explain.md", name: "explain.md", isFile: true, isDirectory: false },
    ],
  };
  const workspace = {
    async walk(options: {
  ...
  expect(resources.promptTemplates).toEqual([
    expect.objectContaining({ name: "explain", description: "Explain selected code" }),
  ]);
});
