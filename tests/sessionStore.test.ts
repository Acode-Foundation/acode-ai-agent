  ...
  const { harness } = await AgentHarness.create(
    {
      ...
      models,
      model: faux.getModel(),
      compaction: { enabled: false, reserveTokens: 1024, keepRecentTokens: 1024 },
    },
    context,
  );
...
test("uses Pi's branch fork, preserving source and excluding selected user prompt", async () => {
  ...
  const selected = entries.find(
    (entry) =>
      ...
      JSON.stringify(entry.message.content).includes("two"),
  )!;
  await store.fork("c1", { ...options("fork"), updatedAt: Date.now() }, selected.id);
  const fork = await store.open(options("fork"));
  const forkEntries = await (await fork.session.branch("main", context))!.findEntries(
    { order: "oldestFirst" },
    context,
  );
  expect(forkEntries).toHaveLength(2);
  expect(await lane.findEntries(undefined, context)).toHaveLength(4);
...
test("exports and imports Pi JSONL through Pi's repository", async () => {
  ...
  expect(
    await (await imported.session.branch("main", context))!.findEntries(undefined, context),
  ).toHaveLength(2);
...
test("repairs an interrupted JSONL tail using Pi and preserves prior transactions", async () => {
  ...
  const next = new SessionStore(adapter);
  const restored = await next.open(options());
  expect(await restored.session.findEntries(undefined, context)).toHaveLength(2);
  expect(await next.export("c1")).not.toContain('"incomplete"');
  await next.release("c1");
});

test("stores tasks in Pi and deletes them with the session", async () => {
  const { store } = await setup();
  const tasks = { tasks: [{ id: "1", subject: "Inspect", status: "pending" }] };
  await store.saveTasks("c1", tasks);
  expect(await store.loadTasks("c1")).toEqual(tasks);
...
test("redacts persisted secrets without changing live messages or image data", async () => {
  ...
  getOrThrow(
    await lane.prompt(
      ...
      [{ type: "image", data: "c2stYWJjZGVmZ2hpamtsbW5vcA==", mimeType: "image/png" }],
      context,
    ),
  );
  const text = await store.export("c1");
  ...
  expect(JSON.stringify(await lane.findEntries(undefined, context))).toContain(
...
test("Pi tree forks retain alternate paths without moving the source tip", async () => {
  ...
  await store.fork(
    "c1",
    { ...options("tree-fork"), updatedAt: Date.now() },
    undefined,
    "main",
    true,
  );
...
test("recovers a timestamp previously serialized as null", async () => {
  const { store, opened, adapter, harness } = await setup();
  await opened.session.setValue(
    value("acode", "metadata"),
    { ...opened.record, updatedAt: null },
    context,
  );
  await harness.close(context);
  await store.release("c1");
