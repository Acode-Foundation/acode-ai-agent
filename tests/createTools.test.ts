...
test("grep applies case sensitivity to regular expressions", async () => {
  ...
  const output = await grep.execute("grep-2", {
    query: "^const",
    regex: true,
    case_sensitive: true,
  });
...
test("grep reports invalid regular expressions before searching", async () => {
  ...
  await expect(grep.execute("grep-4", { query: "[", regex: true })).rejects.toThrow(
...
function fakeWorkspace(
  ...
  return {
    info: {
      id: "workspace",
      name: "project",
      rootUri: remote ? "sftp://workspace/project" : "file:///project",
      scheme: remote ? "sftp" : "file",
      remote,
    },
    ...
    walk: async (options: {
      ...
      for (const path of Object.keys(files)) {
        visited += 1;
        const stop = await options.onEntry({
          path,
