...
export class TaskList {
  ...
  replace(drafts: TaskDraft[]): TaskReplaceResult {
    ...
    if (drafts.length > MAX_TASKS)
      warnings.push(`Kept ${MAX_TASKS} tasks, dropped ${drafts.length - MAX_TASKS}.`);

    const previousList = this.list();
    const incomingHasOpen = incoming.some((draft) => {
      const status =
        typeof draft.status === "string" && isTaskStatus(draft.status) ? draft.status : "pending";
      return status === "pending" || status === "in_progress";
    });
    const rollFinishedList =
      previousList.length > 0 &&
      previousList.every((task) => isResolvedStatus(task.status)) &&
      incomingHasOpen;
    ...
    for (let index = 0; index < incoming.length; index += 1) {
      ...
      next.set(id, {
        ...
        updatedAt:
          previous && sameTask(previous, parsed.subject, parsed.activeForm, status)
            ? previous.updatedAt
            : now,
        startedAt,
      });
...
export function parseTaskStore(value: unknown): TaskStoreData {
  ...
  const nextId =
    typeof record.nextId === "number" && Number.isInteger(record.nextId) && record.nextId > maxId
      ? record.nextId
      : maxId + 1;
...
function parseStoredTask(value: unknown): Task | undefined {
  ...
  const subject =
    typeof record.subject === "string" ? clampText(record.subject, MAX_SUBJECT_CHARS) : "";
  if (!id || !subject) return undefined;
  const status =
    typeof record.status === "string" && isTaskStatus(record.status) ? record.status : "pending";
  const activeForm =
    typeof record.activeForm === "string"
      ? clampText(record.activeForm, MAX_ACTIVE_FORM_CHARS)
      : undefined;
  const blockedBy = Array.isArray(record.blockedBy)
    ? uniqueIds(
        record.blockedBy
          .map((item) => (typeof item === "string" ? normalizeId(item) : undefined))
...
function parseDraft(
  ...
  warnings: string[],
  id: string,
): { subject: string; activeForm?: string; status: TaskStatus } | undefined {
  const subject =
    typeof draft.content === "string"
      ? clampText(
          draft.content.replace(/[\r\n]+/g, " ").replace(/<\/?system-reminder>/gi, ""),
          MAX_SUBJECT_CHARS,
        )
      : "";
  if (!subject) {
  ...
  if (statusRaw && !isTaskStatus(statusRaw))
    warnings.push(`#${id} invalid status "${statusRaw}"; using pending.`);
  const activeForm =
    typeof draft.activeForm === "string"
...
function assignIds(
  ...
  existing: Task[],
  nextId: number,
): { ids: string[]; nextId: number } {
  const used = new Set<string>();
  const ids: string[] = [];
  let cursor = nextId;
  const nameless = drafts.every(
    (draft) => !normalizeId(typeof draft.id === "string" ? draft.id : undefined),
  );
  if (nameless) {
  ...
  for (const task of tasks.values()) {
    for (const blockerId of task.blockedBy) {
      ...
      if (blocker?.blockedBy.includes(task.id))
        warnings.push(`cycle: #${task.id} and #${blockerId} block each other.`);
    }
  }
}
...
function demoteBlockedInProgress(tasks: Map<string, Task>, warnings: string[]): void {
  ...
  for (const task of list) {
    ...
    task.updatedAt = Date.now();
    warnings.push(
      `#${task.id} still pending; blocked by ${blockers.map((id) => `#${id}`).join(", ")}.`,
    );
  }
}
...
function sameTask(
  ...
  return (
    previous.subject === subject && previous.activeForm === activeForm && previous.status === status
  );
}
