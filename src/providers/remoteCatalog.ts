...
export function withRemoteCatalog(baseline: Provider, store: ModelsStore): RemoteCatalog {
  ...
  const apply = (entry: ModelsStoreEntry | undefined) => {
    ...
    remote = (entry?.models ?? [])
      .filter(isModel)
      .map((model) => ({ ...model, provider: baseline.id }));
  };
  const restore = async () => {
    if (restored) return;
  ...
  const refresh = (force = false, signal?: AbortSignal) => {
    inflight ??= (async () => {
      ...
      if (
        signal?.aborted ||
        (!force && stored?.checkedAt && Date.now() - stored.checkedAt < REFRESH_INTERVAL)
      )
        return;
      ...
      try {
        ...
        if (stored?.lastModified)
          headers["If-Modified-Since"] = new Date(stored.lastModified).toUTCString();
        const response = await nativeFetch(
          `https://pi.dev/api/models/providers/${encodeURIComponent(baseline.id)}`,
          {
            headers,
  ...
  return {
    provider: {
      ...
      refreshModels: (context) =>
        context.allowNetwork ? refresh(context.force, context.signal) : restore(),
    },
    restore,
    refresh,
  };
...
function parseModels(value: unknown, provider: string): Model<Api>[] {
  const body =
    value && typeof value === "object" && "models" in value
