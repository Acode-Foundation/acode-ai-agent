...
export const portableGitHubCopilotOAuth: OAuthAuth = {
  ...
  async login(interaction) {
    ...
    if (input.trim() && !enterpriseDomain)
      throw new Error("Enter a valid GitHub Enterprise domain.");
  ...
  async refresh(credential, signal) {
    return githubCopilotCredential(
      credential.refresh,
      credentialEnterpriseDomain(credential),
      signal,
    );
  },
  async toAuth(credential) {
    ...
    return {
      ...
      baseUrl: githubCopilotBaseUrl(credential.access, enterpriseDomain),
    };
  },
};
...
export const portableKimiOAuth: OAuthAuth = {
  ...
  async login(interaction) {
    const device = await postForm(
      `${KIMI_AUTH_BASE}/api/oauth/device_authorization`,
      {
    ...
    const verificationUri = secureUrl(
      optionalString(device, "verification_uri_complete") ??
    ...
    return pollDeviceCode<OAuthCredential>({
      ...
      poll: async () => {
        ...
        if (response.ok && optionalString(response.body, "access_token"))
          return { done: standardOAuthCredential(response.body, undefined, false) };
        return pendingResult(response.body);
      },
    });
  },
...
export const portableXaiOAuth: OAuthAuth = {
  ...
  async login(interaction) {
    ...
    const verificationUri = secureUrl(
      optionalString(response, "verification_uri_complete") ??
        requiredString(response, "verification_uri"),
    );
...
async function codexCredential(
  ...
  const accountId =
    auth && typeof auth === "object"
      ? optionalString(auth as Json, "chatgpt_account_id")
      : undefined;
  ...
  try {
    ...
    return Array.isArray(previousIds) && previousIds.every((id) => typeof id === "string")
      ? {
          ...credential,
          availableModelIds: previousIds,
          availableModelsFetchedAt: previous?.availableModelsFetchedAt,
        }
      : credential;
  }
}
...
export async function refreshPortableCodexModels(
  ...
  const availableModelIds = models.flatMap((value) => {
    ...
    return typeof model.slug === "string" &&
      sanitizeModelId(model.slug) === model.slug &&
  ...
  if (!availableModelIds.length)
    throw new Error(
      "ChatGPT returned no discoverable models. Using the saved Codex catalog; retry model refresh later.",
    );
  return {
    ...credential,
    availableModelIds: [...new Set(availableModelIds)],
...
function standardOAuthCredential(
  body: Json,
  ...
  return {
    ...
    refresh:
      ...
      requiredString(body, "refresh_token"),
    expires:
      Date.now() + positiveNumber(body.expires_in, 3600) * 1000 - (applySkew ? REFRESH_SKEW_MS : 0),
  };
}

async function githubCopilotCredential(
  githubAccess: string,
...
async function githubCopilotModelIds(
  access: string,
  enterpriseDomain?: string,
  signal?: AbortSignal,
): Promise<string[]> {
  ...
  return data.flatMap((value) => {
    ...
    const model = value as Json;
    const policy =
      model.policy && typeof model.policy === "object" ? (model.policy as Json) : undefined;
    const capabilities =
...
function githubCopilotBaseUrl(access: string, enterpriseDomain?: string): string {
  ...
  if (proxyHost && /^[a-z0-9.-]+$/i.test(proxyHost))
...
function credentialEnterpriseDomain(credential: OAuthCredential): string | undefined {
  return typeof credential.enterpriseUrl === "string"
    ? (normalizeDomain(credential.enterpriseUrl) ?? undefined)
    : undefined;
}
...
function pendingResult(body: Json): { pending?: boolean; slowDown?: boolean; error?: string } {
  ...
  if (error === "slow_down") return { slowDown: true };
  if (error === "access_denied" || error === "authorization_denied")
    return { error: "Device authorization was denied." };
...
async function postFormAllowError(
  url: string,
  fields: Record<string, string>,
  signal?: AbortSignal,
) {
...
async function requestJson(
  url: string,
  init: RequestInit,
): Promise<{ ok: boolean; status: number; body: Json; url: string }> {
...
function requiredString(body: Json, field: string): string {
  ...
  if (typeof value !== "string" || !value)
    throw new Error(`Authentication response is missing ${field}.`);
  return value;
}
