...
export function allocateProviderId(
  ...
  const base =
    preferredId && PROVIDER_ID.test(preferredId) && !RESERVED_PROVIDER_IDS.has(preferredId)
      ? preferredId
...
export function sanitizeCustomEndpoint(
  value: unknown,
  taken: Iterable<string> = [],
  ...
  const baseUrl =
    typeof record.baseUrl === "string" ? normalizeOpenAIBaseUrl(record.baseUrl) : undefined;
  if (!name || !baseUrl) return undefined;
  const existingId = typeof record.id === "string" ? record.id.trim().toLowerCase() : undefined;
  if (existingId && (!PROVIDER_ID.test(existingId) || isReservedProviderId(existingId)))
    return undefined;
  const id =
    existingId && ![...taken].includes(existingId)
      ? existingId
...
export function finalizeCustomEndpoint(
  ...
  const taken = existing
    .filter((endpoint) => endpoint.id !== draft.id)
