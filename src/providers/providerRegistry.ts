...
export const PROVIDERS: readonly ProviderDescriptor[] = [
  {
    id: "openrouter",
    name: "OpenRouter",
    hint: "API key or OpenRouter account",
    keyPlaceholder: "sk-or-v1-…",
    keyUrl: "https://openrouter.ai/keys",
    apiKey: true,
    subscriptionLabel: "Sign in with OpenRouter",
  },
  {
    id: "openai",
    name: "OpenAI",
  ...
  },
  {
    id: "xai",
    name: "xAI",
    hint: "API key or Grok subscription",
    keyPlaceholder: "xai-…",
  ...
  },
  {
    ...
    hint: "Alibaba token plan",
    keyPlaceholder: "sk-…",
    keyUrl: "https://www.alibabacloud.com",
    apiKey: true,
  },
  {
    id: "ant-ling",
...
export class ProviderRegistry {
  ...
  getModels(providerId: ProviderId): Model<any>[] {
    ...
    const template = endpoint
      ? customEndpointModel(endpoint, endpoint.models[0] ?? "custom")
      : undefined;
  ...
  async refreshModelAvailability(providerId: ProviderId, force = false): Promise<Model<any>[]> {
    ...
    if (
      ...
      (force ||
        ...
        !credential.availableModelIds.length ||
        typeof credential.availableModelsFetchedAt !== "number" ||
        Date.now() - credential.availableModelsFetchedAt > 60 * 60 * 1000)
    ) {
      try {
        credential = await this.#credentials.modify(providerId, async (current) =>
          current?.type === "oauth" ? refreshPortableCodexModels(current) : undefined,
        );
      } catch (error) {
      ...
      const ids =
        credential?.type === "oauth" && Array.isArray(credential.availableModelIds)
          ? credential.availableModelIds.filter(
              (id): id is string => typeof id === "string" && sanitizeModelId(id) === id,
            )
          : [];
  ...
  resolveModel(providerId: ProviderId, modelId: string): Model<any> {
    ...
    if (id) {
      ...
      return createCustomModel(
        providerId,
        id,
        this.models.getModels(providerId)[0] ??
          (endpoint ? customEndpointModel(endpoint, id) : undefined),
      );
    }
    const fallback = this.getModels(providerId)[0];
  ...
  async refreshModel(providerId: ProviderId, modelId: string, force = false): Promise<Model<any>> {
    ...
    const metadataKnown = (model as Model<any> & { inputModalitiesKnown?: boolean })
      .inputModalitiesKnown;
...
function withNativeFetch(models: MutableModels): MutableModels {
  ...
  models.streamSimple = (model, context, options) =>
    streamSimple(model, context, { ...options, fetch: nativeFetch });
  models.completeSimple = (model, context, options) =>
    completeSimple(model, context, { ...options, fetch: nativeFetch });
  return models;
}
