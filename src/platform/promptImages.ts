...
export function modelAcceptsImages(model?: Model<any>): boolean | undefined {
  ...
  if ((model as Model<any> & { inputModalitiesKnown?: boolean }).inputModalitiesKnown === false)
    return undefined;
...
export async function imageContentFromFile(
  file: File | Blob,
  name = "image",
  autoResize = true,
  ...
  const prepared = autoResize
    ? await resizeImage(file, mime)
    : { data: await blobToBase64(file), mimeType: mime };
  return { type: "image", data: prepared.data, mimeType: prepared.mimeType };
}

export async function imageContentFromBytes(
  bytes: Uint8Array,
  name: string,
  mimeType?: string,
  autoResize = true,
...
export function toPiImages(images: ImageContent[]): ImageContent[] {
  return images
    ...
    .map((image) => ({
      ...
      mimeType: normalizeImageMime(image.mimeType) || "image/jpeg",
    }));
}
...
async function resizeImage(
  ...
  try {
    ...
    const outputType =
      mimeType === "image/png" || mimeType === "image/gif" ? mimeType : "image/jpeg";
