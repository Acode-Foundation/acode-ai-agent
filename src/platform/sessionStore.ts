...
export function createSessionStore(_ctx?: Acode.PluginContext | null): SessionStore {
...
export class SessionStore {
  ...
  async #hydrate(): Promise<void> {
    ...
    this.#repo ??= new JsonlSessionRepo({
      fileSystem: redactedSessionFileSystem(this.#fs),
      ...
      try {
        ...
        if (record)
          this.#index.set(metadata.id, {
            ...
            id: metadata.id,
            updatedAt:
              sessionTimestamp(record.updatedAt) ??
              sessionTimestamp(metadata.modifiedAt) ??
              metadata.createdAt,
          });
      } finally {
  ...
  async open(options: {
    ...
    return {
      session,
      record,
      update: (patch: SessionMetaPatch, activity = true) => {
        Object.assign(record, patch, activity ? { updatedAt: Date.now() } : {});
      },
      persist: async () => {
  ...
  async loadTasks(id: string): Promise<unknown> {
    return (await (await this.#session(id)).getValue(TASKS, context))?.value;
  }
  async saveTasks(id: string, tasks: unknown): Promise<void> {
    await (await this.#session(id)).setValue(TASKS, tasks, context);
  }
  async copyTasks(fromId: string, toId: string): Promise<void> {
    const tasks = await this.loadTasks(fromId);
    if (tasks !== undefined) await this.saveTasks(toId, tasks);
  }
  async fork(
    fromId: string,
    ...
    const session = await this.#repo!.fork(
      source,
      wholeTree
        ? { scope: "tree", id: options.id }
        : {
            scope: "branch",
            branch,
            id: options.id,
            ...(targetId ? { entryId: targetId, position: "before" as const } : {}),
          },
      context,
    );
    this.#metadata.set(options.id, session.metadata);
  ...
  async import(text: string, options: SessionRecord): Promise<void> {
    ...
    try {
      ...
      getOrThrow(await this.#fs!.remove(temp, { recursive: true, force: true }, context));
    }
  }
}
