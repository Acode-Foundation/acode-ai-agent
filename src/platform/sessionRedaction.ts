...
export function redactSessionJsonl(text: string): string {
	return text.split("\n").map((line) => line.trim() ? JSON.stringify(redactDeep(JSON.parse(line))) : line).join("\n");
}
				...
				const write: FileSystem["writeFile"] = async (path, content, context) => {
					try {
						const next = /\.jsonl(?:\.tmp)?$/.test(path)
							? redactSessionJsonl(typeof content === "string" ? content : new TextDecoder().decode(content)) : content;
						return await method(path, next, context);
					} catch (error) { return err(new FileError("unknown", error instanceof Error ? error.message : String(error), path)); }
				};
				return write;
			}
