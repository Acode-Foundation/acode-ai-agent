...
export function resolveWorkspaceUri(path: string, workspace?: WorkspaceInfo): string | undefined {
  ...
  if (!workspace?.rootUri || typeof acode === "undefined" || typeof acode.joinUrl !== "function")
    return undefined;
  try {
    return new PathSandbox(workspace.rootUri, (root, relative) =>
      acode.joinUrl(root, relative),
    ).resolve(cleaned).uri;
  } catch {
    return undefined;
  }
