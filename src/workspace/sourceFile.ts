...
export function isSourceEditorFile(
  file: Acode.EditorFile | null | undefined,
): file is Acode.EditorFile {
...
export function resolveSourceFile(
  preferred?: Acode.EditorFile | null,
): Acode.EditorFile | undefined {
  const files = window.editorManager?.files ?? [];
  if (lastSourceFile && !files.some((file) => file.id === lastSourceFile?.id))
    lastSourceFile = null;
  return [preferred, lastSourceFile, window.editorManager?.activeFile, ...files].find(
    isSourceEditorFile,
  );
}
...
export function selectedText(file: Acode.EditorFile): string {
  try {
    ...
    const session = file.session as
      | {
          getSelection?: () => { getRange?: () => unknown };
          getTextRange?: (range: unknown) => string;
        }
      | undefined;
