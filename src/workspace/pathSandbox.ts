    ...
    for (const candidate of [uriPath, uri]) {
      for (const separator of ["/", "::"]) {
        ...
        if (candidate.startsWith(prefix))
          return this.#safeNormalize(candidate.slice(prefix.length));
      }
    }
    return undefined;
  }
...
function relativeSafPath(
  root: SafDocument | undefined,
  uri: SafDocument | undefined,
): string | undefined {
  ...
  if (root.docId && uri.docId.startsWith(`${root.docId}/`))
    return uri.docId.slice(root.docId.length + 1);
...
function sameWorkspaceUri(
  ...
  return (
    canonicalUri(first.treeUrl) === canonicalUri(second.treeUrl) && first.docId === second.docId
  );
}
...
function validatePathSafety(value: string): void {
  ...
  if (value.includes("?") || value.includes("#"))
    throw new Error("URI query and fragment delimiters are not allowed in tool paths.");
