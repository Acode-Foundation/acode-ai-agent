...
export function getAddedFolders(): Acode.Folder[] {
  ...
  const folders = Array.isArray(fromApi)
    ? fromApi
    : (globalThis as typeof globalThis & { addedFolder?: Acode.Folder[] }).addedFolder;
  if (!Array.isArray(folders)) return [];
  return folders.filter((folder): folder is Acode.Folder =>
    Boolean(folder && typeof folder.url === "string" && folder.url),
  );
}
