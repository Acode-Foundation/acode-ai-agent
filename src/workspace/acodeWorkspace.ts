...
export class AcodeWorkspace {
  ...
  async #listViaFs(base: { relativePath: string; uri: string }): Promise<FileEntry[]> {
    ...
    try {
      ...
      if (indexed) return indexed;
      throw new Error(
        `Cannot list ${base.relativePath || "the workspace"} with Acode's folder APIs.`,
      );
    }
  }
...
function isIgnored(path: string): boolean {
  return path
    .split("/")
    .some((part) => [".git", "node_modules", "dist", "build", ".next", ".cache"].includes(part));
}
