...
export class AgentController {
  ...
  constructor(ctx: Acode.PluginContext | null) {
    this.credentials = new PortableCredentialStore(ctx, () =>
      this.settings.value.customEndpoints.map((endpoint) => endpoint.id),
    );
  ...
  async initialize(): Promise<void> {
    ...
    const availableModels = await this.providers.refreshModelAvailability(
      this.settings.value.providerId,
    );
    if (
      availableModels.length &&
      !availableModels.some((model) => model.id === this.settings.value.modelId)
    ) {
      const model = preferredModel(this.settings.value.providerId, availableModels);
      this.settings.update({
        modelId: model.id,
        thinkingLevel: clampThinkingLevel(model, this.settings.value.thinkingLevel),
      });
    }
    const workspaces = this.workspaces;
    const selected =
      workspaces.find((workspace) => workspace.id === this.settings.value.activeWorkspaceId) ??
      workspaces[0];
    if (selected) await this.selectWorkspace(selected.id);
      ...
      this.#state = {
        ...this.#state,
        status: "ready",
        models: this.providers.getModels(this.settings.value.providerId),
  ...
  async send(
    ...
    const attached = await collectPromptImages(
      prompt,
      images ?? [],
      session.workspace,
      this.settings.value.imageAutoResize,
    );
    if (this.settings.value.blockImages && attached.length)
      throw new Error("Images are blocked in Pi settings.");
    if (!prompt && !attached.length) return;
    const auth = await this.providers.models.checkAuth(this.settings.value.providerId);
    if (!auth)
      throw new Error(
        `Add a ${this.settings.value.providerId} credential before sending a message.`,
      );
    try {
  ...
  async addGlobalSkillRoot(): Promise<
    { skills: string[]; prompts: string[]; roots: string[] } | undefined
  > {
  ...
  async removeGlobalSkillRoot(uri: string): Promise<void> {
    this.settings.update({
      globalSkillRoots: this.settings.value.globalSkillRoots.filter((root) => root !== uri),
    });
  ...
  async executeSlashCommand(name: string, args: string): Promise<SlashCommandExecution> {
    ...
    const command = session.snapshot.commands.find(
    ...
    switch (command.name) {
      ...
      case "tasks": {
        ...
        if (action === "clear") {
          ...
          return {
            message: removed
              ? `Cleared ${removed} task${removed === 1 ? "" : "s"}.`
        ...
        if (action === "clear-completed" || action === "clear completed") {
          ...
          return {
            message: removed
              ? `Cleared ${removed} finished task${removed === 1 ? "" : "s"}.`
      ...
      case "import":
        ...
        return {};
      case "hotkeys":
        return { panel: hotkeysPanel() };
      default:
        throw new Error(`Command /${command.name} is not available in Acode.`);
    }
  }
  ...
  async #selectWorkspace(workspaceIdOrUri: string): Promise<void> {
    await this.#sessionStore.hydrate();
    const info = this.workspaces.find(
      (workspace) => workspace.id === workspaceIdOrUri || workspace.rootUri === workspaceIdOrUri,
    );
  ...
  async saveCustomEndpoint(draft: CustomEndpointDraft, apiKey?: string): Promise<CustomEndpoint> {
    ...
    if (!models.length)
      throw new Error("Add at least one model id, or load the list from /models.");
  ...
  async refreshCustomEndpointModels(
    providerId: ProviderId = this.settings.value.providerId,
  ): Promise<string[]> {
    ...
    this.settings.update({
      customEndpoints: upsertCustomEndpoint(this.settings.value.customEndpoints, {
        ...endpoint,
        models,
      }),
    });
  ...
  removeCustomModel(modelId: string): void {
    const providerId = this.settings.value.providerId;
    const remaining = (this.settings.value.customModels[providerId] ?? []).filter(
      (id) => id !== modelId,
    );
    const customModels = { ...this.settings.value.customModels };
    ...
    const nextId =
      this.settings.value.modelId === modelId
        ? (remaining[0] ??
          this.providers.getModels(providerId).find((model) => model.id !== modelId)?.id)
  ...
  async loginSubscription(providerId: ProviderId): Promise<void> {
    ...
    try {
      ...
      this.#state.authFlow = {
        providerId,
        status: "connected",
        message: "Subscription connected. Choose a model to start chatting.",
      };
      this.#state.error = undefined;
      this.#emit();
    } catch (error) {
  ...
  async hasCredential(providerId: ProviderId): Promise<boolean> {
    ...
    return Boolean(
      endpoint &&
      isLocalUrl(endpoint.baseUrl) &&
      (await this.providers.models.checkAuth(providerId)),
    );
  }
  ...
  #requestAuthPrompt(
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      const onAbort = () =>
        finish(() => reject(new DOMException("Sign-in cancelled", "AbortError")));
      const pending = {
      ...
      this.#state.authFlow = {
        ...
        status: "waiting",
        verificationUri:
          this.#state.authFlow?.providerId === providerId
            ? this.#state.authFlow.verificationUri
            : undefined,
...
function resourcePanel(loaded: {
  ...
  return {
    ...
    body: parts.join("\n\n"),
    rows: loaded.roots.length
      ? [{ label: "Global roots", value: `${loaded.roots.length} scanned` }]
      : undefined,
  };
}
