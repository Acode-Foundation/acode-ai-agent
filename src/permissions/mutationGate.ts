...
export class MutationGate {
  ...
  async request(
    ...
    if (isShell ? this.#allowShellForSession : mode === "allow-edits" || this.#allowEditsForSession)
      return {};
    if (signal?.aborted)
      return {
        block: true,
        reason: isShell
          ? "Terminal command was aborted before approval."
          : "Edit was aborted before approval.",
      };
    if (this.#pending)
      return {
        block: true,
        reason: `Another ${isShell ? "terminal command" : "edit"} approval is already pending.`,
      };
    ...
    return decision === "deny"
      ? {
          block: true,
          reason: isShell
            ? "User denied this terminal command."
            : "User denied this workspace edit.",
        }
      : {};
  }
