...
export async function loadWorkspaceResources(
  ...
  configuredGlobalRoots: string[] = [],
): Promise<LoadedWorkspaceResources> {
  ...
  const [projectAgents, projectPi, projectPrompts, globalSkillGroups, globalPromptGroups] =
    await Promise.all([
...
async function loadProjectSkills(
  ...
  try {
    await workspace.walk({
      ...
      onEntry(entry) {
        ...
        if (
          entry.name === "SKILL.md" ||
          (includeRootMarkdown && !relative.includes("/") && /\.md$/i.test(entry.name))
        )
  ...
  const skills = await Promise.all(
    paths.sort().map((path) => readSkill(workspaceReader(workspace), path)),
  );
...
export async function pickGlobalSkillsFolder(): Promise<{ uri: string; name: string } | undefined> {
  ...
  try {
    ...
    if (/cancel|abort/i.test(error instanceof Error ? error.message : String(error)))
      return undefined;
    throw error;
  }
}
...
function automaticGlobalSkillRoots(): string[] {
  ...
  for (const folder of (globalThis as { addedFolder?: Acode.AddedFolder }).addedFolder ?? []) {
    if (/(?:^|\/)\.(?:agents\/skills|pi\/agent\/skills)\/?$/i.test(folder.url))
      roots.push(folder.url);
  }
  return unique(roots);
}
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    try {
      ...
      prompts.push({
        ...
        description: parsed.values.description || truncate(firstLine, 60),
        content: parsed.body,
      });
    } catch (error) {
