...
export function AskCard({
  ...
  const answered = useMemo(
    () => drafts.map((item, offset) => hasAnswer(questions[offset], item)),
    [drafts, questions],
  );
  ...
  useEffect(() => {
    ...
    scroller.scrollTo({
      left: currentStep.offsetLeft - scroller.clientWidth / 2 + currentStep.offsetWidth / 2,
      behavior: "smooth",
    });
  }, [index]);
  ...
  const patch = (update: Partial<Draft>) => {
    ...
    setDrafts((currentDrafts) =>
      currentDrafts.map((item, offset) => (offset === index ? { ...item, ...update } : item)),
    );
  };
  ...
  const preview =
    !current.multiSelect && draft.selected[0]
    ...
    <section ref={root} class="ask-card" role="dialog" aria-label="Agent question">
      ...
      <div class="ask-body">
        <div class="ask-panel" ref={panel}>
          <p class="ask-question" aria-live="polite">
            {current.question}
          </p>
          ...
          <div class="ask-custom">
            ...
            <Collapse open={draft.customOpen}>
              <textarea
                ...
                value={draft.custom}
                onInput={(event) =>
                  patch({ custom: (event.target as HTMLTextAreaElement).value, customOpen: true })
                }
              />
            </Collapse>
          </div>
