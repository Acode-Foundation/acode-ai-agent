...
export function TreeSheet({
  ...
  const selected =
    rows.find((row) => row.id === selectedId) ?? items.find((item) => item.id === selectedId);
  const selectedIndex = rows.findIndex((row) => row.id === selectedId);
  const branches = useMemo(() => countTreeBranches(items), [items]);
  const turns = useMemo(
    () =>
      items.filter(
        (item) => item.kind === "user" || item.kind === "assistant" || item.kind === "summary",
      ).length,
    [items],
  );
  ...
  useEffect(() => {
    ...
    setSelectedId(
      rows.find((row) => row.current)?.id ??
        [...rows].reverse().find((row) => row.active)?.id ??
        rows[0]!.id,
    );
  }, [rows, selectedId]);
        ...
        <>
          ...
          <div class="tree-list" ref={listRef} role="listbox" aria-label="Session tree">
            {loading ? (
              ...
              <p class="sheet-empty">
                {query.trim() ? "No turns match that search." : "No tree entries match."}
              </p>
            ) : (
              rows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {row.current ? (
                      <em class="tree-now">now</em>
                    ) : isSelected ? (
                      <time>{formatTreeTime(row.timestamp)}</time>
                    ) : null}
                  </div>
                );
              })
          ...
          <footer class="tree-actions">
            <div class="tree-actions-meta">
              {selected ? (
                <>
                  <p class="tree-preview">
                    {selected.label ? `[${selected.label}] ${selected.text}` : selected.text}
                  </p>
                  <small>
                    ...
                    {selected.current
                      ? "Current leaf"
                      : selected.kind === "user" && mode === "tree"
                        ? "Retry this prompt on a new branch"
                        : mode === "fork"
                          ? "New session from this prompt"
            ...
            <div class={`tree-action-buttons${mode === "fork" ? " fork" : ""}`}>
              ...
              {mode === "fork" ? (
                <button class="primary" type="button" disabled={!canGo} onClick={() => fork(close)}>
                  {primaryLabel}
                </button>
              ) : (
                <>
...
function initialSelection(items: SessionTreeItem[], mode: Mode): SessionTreeItem | undefined {
  if (mode === "fork")
    return [...items].reverse().find((item) => item.kind === "user" && item.active);
  return (
    items.find(
      (item) =>
        item.current &&
        (item.kind === "user" || item.kind === "assistant" || item.kind === "summary"),
    ) ??
    [...items]
      .reverse()
      .find(
