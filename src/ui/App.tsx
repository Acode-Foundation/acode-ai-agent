...
export function App({ controller, onActiveChatChange }: Props) {
  ...
  const turns = useMemo(() => {
    const built = buildTurns(
      state.messages,
      running ? state.streamingMessage : undefined,
      state.activities,
      running,
    );
    ...
    return built.map((turn) => ({
      ...turn,
      work: turn.work.filter((item) => item.type !== "thinking"),
    }));
  }, [
    state.messages,
  ...
  const send = useCallback(
    async (draft: ComposerDraft, mode: "steer" | "followUp" = "steer") => {
    ...
    <div class="agent-shell">
      ...
      <main class="conversation" ref={scrollRef}>
        {turns.length === 0 ? (
          ...
          <div class="thread">
            ...
            {running &&
              !state.compacting &&
              !turns.some((turn) => turn.streaming) &&
              !turns.at(-1)?.answer && <WorkingIndicator label={workingLabel} />}
          </div>
        )}
      ...
      {piSettingsOpen && (
        <PiSettingsSheet
          controller={controller}
          state={state}
          onClose={() => setPiSettingsOpen(false)}
          onCredentials={() => {
...
function ChatSheet({
  ...
  useEffect(() => {
    if (filter !== "all" && !workspaces.some((workspace) => workspace.id === filter))
      setFilter("all");
  }, [filter, workspaces]);
  const chats = useMemo(
    () =>
          ...
          <div class="chats-body">
            ...
            {chats.length === 0 ? (
              <div class="chat-empty">
                ...
                {state.workspace && (
                  <button class="chat-empty-new" type="button" onClick={() => startNew(close)}>
                    Start a session
                  </button>
                )}
              </div>
                        ...
                        >
                          <span class="chat-copy">
                            ...
                            <small class="chat-meta">
                              {filter === "all" && (
                                <>
                                  <span class="chat-project">
                                    {workspaceLabel(chat, workspaces)}
                                  </span>
                              ...
                              {chat.id === state.activeChatId ? (
                                <span class="chat-current">· current</span>
                              ) : null}
                            </small>
                          </span>
  ...
  if (native) {
    return (
      <label class="project-select">
        ...
        <select
          value={value}
          aria-label="Filter sessions by folder"
      ...
      {hasWorkspace && (
        <div class="suggestions" ref={suggestions}>
          <button
            type="button"
            onClick={() =>
              onPrompt("Map this project and explain how the main pieces fit together.")
            }
          >
            Map the project
          </button>
          <button
            type="button"
            onClick={() => onPrompt("Review the active file and fix the highest-impact issue.")}
          >
...
function SettingsSheet({
  ...
  const [providerId, setProviderId] = useState<ProviderId>(
    providers.some((item) => item.id === state.settings.providerId)
  ...
  useBackAction(
    backActionId("settings-endpoint"),
        ...
        endpointView ? (
          ...
          <>
            ...
            <div class="credential">
              ...
              {selectedEndpoint && (
                <button
                  class="text-button"
                  type="button"
                  onClick={() => setEndpointView({ endpoint: selectedEndpoint })}
                >
                  Edit endpoint
                </button>
              )}
            ...
            <label class="toggle">
              ...
              <input
                type="checkbox"
                checked={state.settings.includeSelection}
                onChange={(event) =>
                  controller.settings.update({ includeSelection: event.currentTarget.checked })
                }
              />
            </label>
            <button
            ...
            >
              <div>
                <b>Pi settings</b>
                <small>Compaction, skills, message delivery, and display</small>
              </div>
        ...
        <>
          ...
          <div class="pi-settings-body">
            ...
            <SettingsToggle
              ...
              hint="Resize large images for model compatibility"
              checked={settings.imageAutoResize}
              onChange={(value) => update({ imageAutoResize: value })}
            />
            <SettingsToggle
              label="Skill commands"
              hint="Register discovered skills as /skill:name"
              checked={settings.enableSkillCommands}
              onChange={(value) => update({ enableSkillCommands: value })}
            />
            <SettingsSelect
              label="Steering mode"
              hint="How messages sent during a run are delivered"
              value={settings.steeringMode}
              options={[
                { value: "one-at-a-time", label: "One at a time" },
                { value: "all", label: "All" },
              ]}
              onChange={(value) => update({ steeringMode: value as typeof settings.steeringMode })}
            />
            <SettingsSelect
              label="Follow-up mode"
              hint="How queued follow-ups are delivered"
              value={settings.followUpMode}
              options={[
                { value: "one-at-a-time", label: "One at a time" },
                { value: "all", label: "All" },
              ]}
              onChange={(value) => update({ followUpMode: value as typeof settings.followUpMode })}
            />
            <SettingsToggle
              label="Hide thinking"
              hint="Hide thinking blocks in assistant responses"
              checked={settings.hideThinkingBlock}
              onChange={(value) => update({ hideThinkingBlock: value })}
            />
            <SettingsSelect
              label="Autocomplete max items"
              hint="Visible items in the @ and / picker"
              value={String(settings.autocompleteMaxVisible)}
              options={[7, 10, 15, 20].map((value) => ({
                value: String(value),
                label: String(value),
              }))}
              onChange={(value) => update({ autocompleteMaxVisible: Number(value) })}
            />
            ...
            <section class="settings-section">
              <header>
                <b>Skills</b>
                <small>
                  Project skills are detected automatically from .pi/skills and .agents/skills
                </small>
              </header>
              {settings.globalSkillRoots.map((root) => (
                <div class="skill-root" key={root}>
                  ...
                  <button
                    type="button"
                    aria-label="Remove skills folder"
                    onClick={() =>
                      void controller
                        .removeGlobalSkillRoot(root)
                        .catch((error) => onToast(String(error)))
                    }
                  >
                    <Trash2 size={14} strokeWidth={2} />
              ...
              <button class="settings-action" type="button" onClick={addSkills}>
                <Folder size={15} strokeWidth={2} /> Add global skills folder
              </button>
            </section>
            <button class="config-nav" type="button" onClick={onCredentials}>
              <div>
                <b>Provider credentials</b>
                <small>Sign in or manage API keys</small>
              </div>
...
function SettingsSelect({
  ...
  const choose = () => {
    void pickAcodeSelect(
      label,
      options.map((option) => ({ value: option.value, text: option.label })),
      value,
    ).then((next) => {
  ...
  return (
    <div class="setting-row">
      ...
      <button class="setting-choice" type="button" onClick={choose}>
        <span>{selected}</span>
...
function CommandResultSheet({
  ...
  const copy = () => {
    void navigator.clipboard
      .writeText(panel.copyText ?? panel.body ?? "")
      .then(() => onToast("Copied"));
  };
  return (
    <Sheet class="command-result" onClose={onClose}>
      {(close) => (
        <>
          ...
          <div class="command-result-body">
            {panel.rows?.map((row) => (
              <div class="result-row" key={row.label}>
                <span>{row.label}</span>
                <b>{row.value}</b>
              </div>
            ))}
            {panel.body &&
              (panel.markdown ? <Markdown text={panel.body} /> : <pre>{panel.body}</pre>)}
            {(panel.copyText || panel.body) && (
              <button class="settings-action" type="button" onClick={copy}>
                Copy
              </button>
            )}
          </div>
              ...
              <>
                ...
                <div class="config-row">
                  <div>
                    ...
                    <small>
                      {
                        PERMISSION_MODES.find((mode) => mode.id === state.settings.permissionMode)
                          ?.hint
                      }
                    </small>
                  </div>
                  <div class="config-pills">
                    {PERMISSION_MODES.map((mode) => (
                      <button
                        type="button"
                        class={state.settings.permissionMode === mode.id ? "selected mode" : ""}
                        key={mode.id}
                        onClick={() => controller.setPermissionMode(mode.id)}
                      >
                        {mode.label}
                      </button>
                    ))}
                  ...
                  <div class="config-row">
                    ...
                    <div class="config-pills">
                      {effortLevels.map((level) => (
                        <button
                          type="button"
                          class={state.settings.thinkingLevel === level.id ? "selected effort" : ""}
                          key={level.id}
                          onClick={() => controller.setThinkingLevel(level.id)}
                        >
                          {level.label}
                        </button>
                      ))}
                ...
                <button type="button" class="config-nav" onClick={() => go("providers")}>
                  ...
                  <span class="config-nav-value">
                    {provider?.name ?? state.settings.providerId}
                  </span>
                  <ChevronRight size={18} strokeWidth={2} aria-hidden="true" />
                </button>
...
function ProviderPicker({
  ...
  const filtered = useMemo(() => {
    ...
    return providers.filter(
      (provider) =>
        !needle ||
        `${provider.name} ${provider.id} ${provider.hint}`.toLowerCase().includes(needle),
    );
  }, [providers, query]);
    ...
    <>
      ...
      <div class="model-list">
        {filtered.map((provider) => (
          <div
            class={`model-row${state.settings.providerId === provider.id ? " selected" : ""}`}
            key={provider.id}
          >
            <button type="button" onClick={() => onPick(provider.id)}>
              ...
              <small>
                {state.settings.providerId === provider.id ? (
        ...
        {filtered.length === 0 && !showCustom && (
          <p class="custom-model-hint">No providers match that search.</p>
        )}
      </div>
    </>
  );
...
function EndpointForm({
  ...
  const [models, setModels] = useState(
    [...new Set([...(endpoint?.models ?? []), ...extraIds])].join("\n"),
  );
  const [busy, setBusy] = useState<"save" | "probe" | "remove" | "">("");
  const [error, setError] = useState("");
  const urlForHints = baseUrl.includes("://")
    ? baseUrl
    : baseUrl.trim()
      ? `http://${baseUrl.trim()}`
      : "";
  ...
  const probe = () => {
    setBusy("probe");
    setError("");
    void probeOpenAIModels(baseUrl, apiKey)
      .then((ids) => {
        setModels((current) =>
          [
            ...new Set([
  ...
  return (
    ...
    >
      ...
      <p class="custom-model-hint">
        Point at the OpenAI-compatible `/v1` URL on your computer. Replace `192.168.1.10` with that
        machine's LAN IP.
      </p>
      {loopback && (
        <p class="endpoint-warn">
          localhost on Android is this phone, not your computer. Use the computer's LAN IP instead.
        </p>
      )}
      <label class="field-label">
        ...
        <input
          ...
          value={apiKey}
          placeholder={
            endpoint ? "Replacement key" : local ? "Leave blank for local servers" : "sk-…"
          }
          autoCapitalize="none"
          autoCorrect="off"
      ...
      <button
        class="text-button"
        type="button"
        disabled={Boolean(busy) || !baseUrl.trim()}
        onClick={probe}
      >
      ...
      <div class="endpoint-actions">
        <button
          class="endpoint-save"
          type="submit"
          disabled={Boolean(busy) || !name.trim() || !baseUrl.trim()}
        >
...
function ModelPicker({
  ...
  const endpoint = state.settings.customEndpoints.find(
    (item) => item.id === state.settings.providerId,
  );
  const filtered = useMemo(() => {
    const needle = query.trim().toLowerCase();
    return state.models.filter(
      (model) => !needle || `${model.name} ${model.id}`.toLowerCase().includes(needle),
    );
  }, [query, state.models]);
  const addCustom = () => {
    try {
  ...
  return (
    <>
      <div class="custom-model">
        ...
        />
        <button type="button" disabled={!query.trim()} onClick={addCustom}>
          Use this id
        </button>
      </div>
      <p class="custom-model-hint">
      ...
      <div class="model-list">
        {filtered.map((model) => {
          ...
          return (
            <div
              class={`model-row${state.model?.id === model.id ? " selected" : ""}`}
              key={model.id}
            >
              <button
                type="button"
              ...
              {custom && (
                <button
                  ...
                  onClick={() => controller.removeCustomModel(model.id)}
                >
        ...
        {filtered.length === 0 && query.trim() && (
          <button class="use-query" type="button" onClick={addCustom}>
            Use “{query.trim()}”
          </button>
        )}
      </div>
...
function JumpLatest({ visible, onJump }: { visible: boolean; onJump: () => void }) {
  ...
  useLayoutEffect(() => {
    ...
    void fadeSlide(element, visible, first.current).then(() => {
      if (!visible && element.getAttribute("aria-hidden") === "true")
...
function ApprovalPanel({
  ...
  onApprove: (decision: MutationDecision) => void;
}) {
  ...
  return (
    <section ref={ref} class="approval" role="alertdialog" aria-label="Approve agent action">
      ...
      <div class="approval-actions">
        <button type="button" onClick={() => onApprove("deny")}>
          Deny
        </button>
        <button type="button" onClick={() => onApprove("allow-session")}>
          Allow this session
        </button>
        <button class="primary" type="button" onClick={() => onApprove("allow")}>
          Allow once
        </button>
      </div>
    </section>
...
function workspaceLabel(chat: ChatSummary, workspaces: WorkspaceInfo[]): string {
  return (
    workspaces.find((workspace) => workspace.id === chat.workspaceId)?.name ||
    chat.workspaceName ||
    "Closed folder"
  );
}
...
function groupChatsByRecency(
  chats: ChatSummary[],
): Array<{ id: string; label: string; chats: ChatSummary[] }> {
  const now = new Date();
  const startToday = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
  const buckets = {
