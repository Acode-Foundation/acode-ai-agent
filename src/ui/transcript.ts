...
export function buildTurns(
  ...
  if (streamingMessage && !transcript.includes(streamingMessage)) {
    ...
    if (
      last &&
      last.role === streamingMessage.role &&
      "timestamp" in last &&
      last.timestamp === streamingMessage.timestamp
    ) {
  ...
  const flushBucket = () => {
    ...
    turns.push(
      projectTurn(
        bucket,
        isRunning && streamingMessage !== undefined && bucket.includes(streamingMessage),
      ),
    );
    bucket = [];
  };
  ...
  if (last && isRunning && isLiveCurrentTurn(last, transcript)) {
    ...
    const priorWorkIds = new Set(
      turns.slice(0, -1).flatMap((turn) => turn.work.map((entry) => entry.id)),
    );
...
function isLiveCurrentTurn(turn: ChatTurn, transcript: AgentMessage[]): boolean {
  ...
  if (
    ...
    lastMessage.content.some((part) => part.type === "toolCall")
  )
    return true;
...
export function presentTool(
  ...
  args: Record<string, unknown> = {},
  output?: string,
): { kind: ToolKind; label: string; detail?: string } {
  ...
  switch (name) {
    ...
    case "web_search":
      return {
        ...
        detail: webSearchDetail(args, query, output),
      };
    case "fetch_content":
      return {
        kind: "web",
    ...
    case "bash":
      return {
        kind: "terminal",
        label: "Ran command",
        detail: inlinePreview(firstString(args, ["command", "cmd"])),
      };
    case "thinking":
...
export type WorkGroup =
  | { kind: "actions"; entries: WorkEntry[] }
  | { kind: "content"; entry: WorkEntry };
...
export function splitWorkBurst(
  entries: WorkEntry[],
  live: boolean,
): { featured: WorkEntry[]; grouped: WorkEntry[] } {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    if (part.kind === "result") {
      ...
      if (existing) {
        ...
        existing.detail =
          presentTool(part.name, existing.args ?? {}, part.text).detail ?? existing.detail;
      } else {
    ...
    if (message.role === "assistant" && Array.isArray(message.content)) {
      for (const part of message.content) {
        if (part.type === "thinking" && part.thinking.trim())
          ...
          parts.push({ kind: "tool", id: part.id, name: part.name, args: part.arguments ?? {} });
  ...
  for (const activity of activities) {
    ...
    if (existing) {
      ...
      if (activity.summary && (!existing.output || existing.status === "running"))
        existing.output = activity.summary;
      existing.detail =
        presentTool(activity.name, activity.args, existing.output).detail ?? presented.detail;
      continue;
    }
...
function todoWriteDetail(args: Record<string, unknown>, output?: string): string | undefined {
  const summary = output
    ...
    .find((line) => line.trim())
    ?.trim();
  if (summary && !summary.startsWith("#") && !summary.startsWith("warning:"))
...
export function splitReadOutput(output?: string): { body: string; notice?: string } {
  ...
  const leading =
    /^\[((?:Showing lines |\d+ more lines in file|Line \d+ exceeds)[^\]]*)\]\n\n/.exec(output);
