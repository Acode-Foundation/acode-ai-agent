...
export function SidebarApp({
  ...
  const scopeLabel =
    scope === "all"
      ? "All projects"
      : (workspaces.find((workspace) => workspace.id === scope)?.name ?? "All projects");
    ...
    <div class="acode-agent-sidebar">
      <header class="as-head">
        ...
        <div>
          ...
          <small>
            {runningCount
              ...
              : (state.workspace?.name ?? "No active project")}
          </small>
        </div>
        <div class="as-head-actions">
          <button
            ...
            title="New session"
          >
            {pending?.startsWith("new:") ? (
              <LoaderCircle class="as-spin" size={15} />
            ) : (
              <Plus size={16} />
            )}
          </button>
      ...
      <div class="as-scroll scroll">
          ...
          {foldersOpen ? (
            <div class="as-projects scroll">
              {workspaces.length ? (
                workspaces.map((workspace) => (
                  <div
                    class={`as-project${workspace.id === state.workspace?.id ? " active" : ""}`}
                    key={workspace.id}
                  >
                    <button
                      class="as-project-select"
                      type="button"
                      onClick={() => setScope(workspace.id)}
                      title={`Show ${workspace.name} sessions`}
                    >
                      <Folder size={14} />
                      <span>
                        <b>{workspace.name}</b>
                        <small>{counts.get(workspace.id) ?? 0} sessions</small>
                      </span>
                    </button>
                    <button
                      ...
                      disabled={Boolean(pending)}
                      onClick={() =>
                        void run(`new:${workspace.id}`, () => onNewSession(workspace.id))
                      }
                      aria-label={`Start session in ${workspace.name}`}
                    >
                      {pending === `new:${workspace.id}` ? (
                        <LoaderCircle class="as-spin" size={14} />
                      ) : (
        ...
        >
          <span>
            {pending === "project" ? (
              <LoaderCircle class="as-spin" size={18} />
            ) : (
              <Sparkles size={18} />
            )}
          </span>
          <span>
            <b>Spin up new session</b>
            <small>Start in a random project</small>
          </span>
        </button>
                      ...
                      <div class={`as-session${chat.running ? " running" : ""}`} key={chat.id}>
                        <button
                          ...
                          title={
                            available
                              ? `Open ${chat.title}`
                              : "Open this project in Acode to resume"
                          }
                        >
                          ...
                          <span>
                            <b>{chat.title}</b>
                            <small>
                              {chat.workspaceName || "Closed project"} ·{" "}
                              {chat.running ? "running" : formatAge(chat.updatedAt)}
                            </small>
                          </span>
                        </button>
                        <div class="as-session-actions">
                          <button
                            ...
                            disabled={!available || Boolean(pending)}
                            onClick={() =>
                              void run(`tab:${chat.id}`, () => onOpenSessionInNewTab(chat.id))
                            }
                          ...
                          {confirmId === chat.id ? (
                            <button
                              class="as-delete confirm"
                              type="button"
                              onClick={() =>
                                void run(`delete:${chat.id}`, async () => {
...
export function filterChats(
  ...
  for (const chat of chats) {
    ...
    if (needle && !`${chat.title} ${chat.workspaceName}`.toLocaleLowerCase().includes(needle))
      continue;
    filtered.push(chat);
  }
