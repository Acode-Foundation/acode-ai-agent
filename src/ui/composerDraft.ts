...
const TOKEN =
  /@((?:[\w.-]+\/)+[\w.-]+|[\w.-]+\.[A-Za-z][\w.-]*)|\[#image\s+([^\]]+)\]|\[#file\s+([^\]]+)\]|\[paste #(\d+)( (\+\d+ lines|\d+ chars))?\]/g;
const ATTACHMENTS = /\n*<attached_files>\n[\s\S]*?\n<\/attached_files>\s*$/;
...
export function splitUserText(
  text: string,
): Array<Extract<UserPart, { type: "text" | "file" | "attachment" | "imageRef" | "paste" }>> {
  const parts: Array<
    Extract<UserPart, { type: "text" | "file" | "attachment" | "imageRef" | "paste" }>
  > = [];
...
export function userPartsFromMessage(
...
export function promptTextFromDraft(draft: ComposerDraft): string {
  ...
  const packed = JSON.stringify(
    draft.files.map(({ name, content, encoding, truncated, kind, pasteId }) => ({
      name,
      content,
      encoding,
      truncated,
