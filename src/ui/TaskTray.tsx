  ...
  return (
    <div class="task-tray" ref={root}>
      ...
      <button
        ...
        class="task-tray-hit"
        onClick={onOpen}
        aria-label={`Tasks, ${counts.resolved} of ${counts.total} done. ${label}`}
      >
              ...
              {counts.resolved > 0 && (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (removed)
                      onToast(`Cleared ${removed} finished task${removed === 1 ? "" : "s"}.`);
                  }}
                >
      ...
      <Collapse open={open}>
        <div class="task-item-actions">
          {task.status !== "skipped" && task.status !== "completed" && (
            <button type="button" aria-label="Skip" onClick={() => onStatus("skipped")}>
              <SkipIcon />
            </button>
          )}
          <button
...
function TaskMark({ status }: { status: string }) {
  if (status === "completed")
    return <Check class="task-mark completed" size={16} strokeWidth={2.4} aria-hidden="true" />;
  if (status === "in_progress")
    return (
      <LoaderCircle class="task-mark in_progress" size={16} strokeWidth={2.4} aria-hidden="true" />
    );
