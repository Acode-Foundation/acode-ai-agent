...
export const Composer = forwardRef<ComposerHandle, Props>(function Composer(props, ref) {
  ...
  const used = props.contextWindow
  ...
  const commandHits = filterSlashCommands(props.commands, commandQuery ?? "").slice(
    0,
    props.autocompleteMaxVisible,
  );
  ...
  const sync = useCallback(() => {
    ...
    const nextEmpty =
      !draft.text
        ...
        .replace(/\[paste #\d+( (?:\+\d+ lines|\d+ chars))?\]/g, "")
        .trim() &&
  ...
  const insertCommand = (command: SlashCommand) => {
    paintDraft(
      editor.current,
      { text: `/${command.name} `, images: [], files: [] },
      attachments.current,
      chipHandlers(),
    );
    setCommandQuery(null);
  ...
  const addFiles = async (files: File[]) => {
    ...
    try {
      ...
      for (const file of chosen) {
        const image = await imageContentFromFile(
          file,
          file.name || "image",
          props.controller.settings.value.imageAutoResize,
        );
    ...
    <div class="mention-menu command-menu" role="listbox" aria-label="Slash commands">
      {commands.length === 0 ? (
        ...
        commands.map((command, index) => (
          ...
          >
            <span class={`command-glyph ${command.source}`} aria-hidden="true">
              /
            </span>
            <span class="mention-name">{highlightMatch(command.name, query)}</span>
            <span class="mention-dir">{command.description}</span>
            {command.source !== "action" && (
              <span class={`command-source ${command.source}`}>{command.source}</span>
            )}
          </button>
        ))
      )}
    ...
    <article class="bubble user">
      <div class="user-body">
        {items.map((part, index) => {
                ...
                onClick={() => {
                  if (content)
                    onPreviewAttachment?.({
                      name,
                      content,
                      encoding: part.type === "attachment" ? part.encoding : "text",
                    });
                }}
              >
          ...
          return (
            <span class="user-text" key={`text-${index}`}>
              {part.text}
            </span>
          );
        })}
  ...
  const walk = (node: Node) => {
    ...
    if ((node.dataset.chip === "attachment" || node.dataset.chip === "paste") && node.dataset.id) {
      ...
      if (file && !("type" in file)) {
        files.push(file);
        chunks.push(
          file.kind === "paste" && typeof file.pasteId === "number"
...
function paintDraft(
  ...
  for (const part of splitUserText(draft.text)) {
    ...
    if (part.type === "paste") {
      const file =
        takeFile(unusedFiles, (item) => item.kind === "paste" && item.pasteId === part.id) ??
...
function createDraftFileChip(file: DraftFile, handlers: ChipHandlers): HTMLSpanElement {
  return file.kind === "paste"
    ? createPasteChip(file, handlers)
    : createAttachedFileChip(file, handlers);
}
  ...
  try {
    ...
    setCaret(
      ...
      nodes[nodes.length - 1]?.nodeType === Node.TEXT_NODE
        ? (nodes[nodes.length - 1] as Text).length
        : 0,
    );
  }
}

function tryDeleteChip(
  root: HTMLElement | null,
  store: Map<string, DraftImage | DraftFile>,
