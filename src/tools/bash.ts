  ...
  return {
    ...
    execute: async (_id, params, signal, onUpdate) => {
      ...
      try {
        ...
        if (exitCode !== 0) {
          const text =
            formatSnapshot(snapshot).content.find((part) => part.type === "text")?.text ?? "";
  ...
  try {
    return await new Promise<number>((resolve, reject) => {
      ...
      const stopStarted = (id: string) => {
        void Promise.resolve(executor.stop(id)).then(
          () => finish(1),
          (error) => {
            if (settled) return;
            settled = true;
...
class BashOutputBuffer {
  ...
  append(value: string): void {
    ...
    while (
      this.#lines.length > DEFAULT_MAX_LINES + 1 ||
      (this.#retainedBytes > DEFAULT_MAX_BYTES * 2 && this.#lines.length > 1)
    ) {
  ...
  snapshot(): { content: string; truncation?: BashTruncation } {
    ...
    for (
      ...
      index >= 0 && selected.length < DEFAULT_MAX_LINES;
      index -= 1
    ) {
