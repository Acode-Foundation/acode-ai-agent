...
export function formatSearchResponse(response: SearchResponse, query: string): string {
  const lines = [
    `Web search via ${response.provider === "webview" ? "device" : response.provider} · “${query}”`,
    "",
  ];
...
export function parseWebSearchOutput(
  output?: string,
): { provider?: string; query?: string; answer?: string; results: SearchResult[] } | undefined {
  ...
  const answer = (
    sourcesAt >= 0
      ? output.slice(header?.[0].length ?? 0, sourcesAt)
      : output.slice(header?.[0].length ?? 0)
  )
    .replace(/^\n+/, "")
    .trim();
