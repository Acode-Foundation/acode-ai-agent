...
export function createWebSearchContext(options: {
  ...
  return {
    ...
    resolveAuth: async (providerId, preferredModelId) =>
      resolveAuth(options.models, providerId, preferredModelId),
  };
}
...
): Promise<WebAuth | undefined> {
  const catalog = models.getModels(providerId);
  const model =
    (preferredModelId ? catalog.find((item) => item.id === preferredModelId) : undefined) ??
    catalog[0];
