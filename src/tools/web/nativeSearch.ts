...
async function searchOpenAI(
  ...
  const auth =
    (await ctx.resolveAuth(providerId, ctx.currentModelId())) ??
  ...
  if (!auth?.apiKey)
    throw new SearchError(
      "openai",
      "credential",
      "OpenAI web search needs an OpenAI or Codex credential.",
    );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new SearchError(
        ...
        `OpenAI search error ${response.status}: ${body}`,
        response.status,
      );
    }
...
async function searchGemini(
  ...
  if (!auth?.apiKey)
    throw new SearchError(
      "google",
      "credential",
      "Gemini web search needs a Google AI Studio key.",
    );
  ...
  try {
    const response = await webRequest(endpoint, {
      ...
      body: JSON.stringify({
        contents: [
          { role: "user", parts: [{ text: `${searchInstructions(options)}\n\n${query}` }] },
        ],
        tools: [{ google_search: {} }],
      }),
      signal: options.signal,
    ...
    if (!response.ok) {
      ...
      throw new SearchError(
        "google",
        classifyHttpError(response.status, body),
        `Gemini search error ${response.status}: ${body}`,
        response.status,
      );
    }
    ...
    if (!answer && !results.length)
      throw new SearchError(
        "google",
        "invalid-response",
        "Gemini search returned no answer or sources.",
      );
...
async function searchXai(
  ...
  if (!auth?.apiKey)
    throw new SearchError(
      "xai",
      "credential",
      "xAI web search needs a Grok API key or subscription.",
    );
  try {
    const response = await webRequest(
      auth.baseUrl ? `${auth.baseUrl.replace(/\/$/, "")}/responses` : XAI_RESPONSES,
      {
        method: "POST",
        headers: {
          ...auth.headers,
    ...
    if (!response.ok) {
      ...
      throw new SearchError(
        "xai",
        classifyHttpError(response.status, body),
        `xAI search error ${response.status}: ${body}`,
        response.status,
      );
    }
    const parsed = parseResponsesBody(await response.text());
    ...
    if (!answer && !results.length)
      throw new SearchError(
        "xai",
        "invalid-response",
        "xAI web search returned no answer or sources.",
      );
...
async function searchAnthropic(
  ...
  if (!auth?.apiKey)
    throw new SearchError(
      "anthropic",
      "credential",
      "Anthropic web search needs an Anthropic API key.",
    );
  try {
    const response = await webRequest(
      auth.baseUrl ? `${auth.baseUrl.replace(/\/$/, "")}/v1/messages` : ANTHROPIC_MESSAGES,
      {
        method: "POST",
        headers: {
          ...auth.headers,
    ...
    if (!response.ok) {
      ...
      throw new SearchError(
        "anthropic",
        classifyHttpError(response.status, body),
        `Anthropic search error ${response.status}: ${body}`,
        response.status,
      );
    }
      ...
      if (block.type === "web_search_tool_result" && Array.isArray(block.content)) {
        for (const item of block.content) {
          ...
          addResult(
            ...
            record.url,
            record.title,
            typeof record.page_age === "string" ? record.page_age : "",
          );
        }
      }
    }
    if (!answers.length && !results.length)
      throw new SearchError(
        "anthropic",
        "invalid-response",
        "Anthropic web search returned no answer or sources.",
      );
    return {
      provider: "anthropic",
      answer: answers.join("\n").trim(),
      results: results.slice(0, normalizeCount(options.numResults)),
    };
  } catch (error) {
  ...
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    try {
      ...
      if (
        ...
        Array.isArray((parsed as { output?: unknown }).output)
      ) {
    ...
    for (const part of content) {
      ...
      const text =
        typeof (part as { text?: unknown }).text === "string"
          ...
          : "";
      for (const annotation of Array.isArray((part as { annotations?: unknown }).annotations)
        ...
        addResult(
          ...
          record.url,
          record.title,
          snippetAround(text, record.start_index, record.end_index),
        );
      }
    }
  }
...
export function authFromResolved(
