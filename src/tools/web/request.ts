...
export function classifyHttpError(status: number, body: string): SearchErrorKind {
  ...
  if (status === 401 || status === 403)
    return /quota|credit|limit|billing/.test(lower) ? "quota" : "auth";
...
export function wrapSearchError(
  ...
  const kind = Number.isFinite(status)
    ...
    : /fetch failed|network|failed to fetch|econnreset|enotfound|etimedout/.test(
          message.toLowerCase(),
        )
      ? "network"
...
const JUNK_HOST =
  /^(?:www\.)?(?:duckduckgo\.com|bing\.com|google\.com|googleadservices\.com|doubleclick\.net|yahoo\.com|yandex\.(?:com|ru)|baidu\.com)$/i;
const TRACKING_PARAM =
  /^(utm_|fbclid$|gclid$|mc_|ref$|ref_src$|spm$|si$|ved$|oq$|source$|sclient$)/i;
