...
const BLOCKED_HINT =
  /just a moment|enable javascript|attention required|verify you are human|captcha|access denied|cf-browser-verification|checking your browser/i;
...
export function htmlToReadable(html: string, url: string): { title: string; content: string } {
  ...
  return {
    ...
    content: collapseBlank(markdown).slice(0, MAX_INLINE_CHARS),
  };
}
...
function pickArticle(html: string): string {
  ...
  for (const tag of ["article", "main"]) {
    for (const match of cleaned.matchAll(
      ...
      const score =
        chunk.length +
        (chunk.match(/<p\b/gi)?.length ?? 0) * 400 +
        (chunk.match(/<h[1-3]\b/gi)?.length ?? 0) * 200;
      if (score > bestScore) {
        best = chunk;
        bestScore = score;
      }
...
function stripNoise(html: string): string {
  return html
    ...
    .replace(/<noscript\b[\s\S]*?<\/noscript>/gi, " ")
    .replace(
      /<(?:svg|iframe|canvas|form|button|input|nav|footer|header)\b[\s\S]*?<\/(?:svg|iframe|canvas|form|button|nav|footer|header)>/gi,
      " ",
    )
