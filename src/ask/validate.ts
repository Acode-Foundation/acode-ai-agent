...
export type ValidationResult =
  | { ok: true; questions: QuestionData[] }
  ...
  for (const item of raw) {
    ...
    if (Array.isArray(value.options)) {
      for (const option of value.options) {
        ...
        const preview =
          typeof entry.preview === "string" && entry.preview.trim() ? entry.preview : undefined;
...
export function validateQuestionnaire(questions: QuestionData[]): ValidationResult {
  ...
  if (questions.length > MAX_QUESTIONS)
    return fail("too_many_questions", "Error: need 1-4 questions.");

  const seenQuestions = new Set<string>();
  for (const question of questions) {
    if (!question.question)
      return fail("empty_question", "Error: each question needs question text.");
    if (seenQuestions.has(question.question))
      return fail("duplicate_question", "Error: question text must be unique.");
    seenQuestions.add(question.question);
    if (question.header.length > MAX_HEADER_LENGTH) {
      return fail(
        "header_too_long",
        `Error: header must be at most ${MAX_HEADER_LENGTH} characters.`,
      );
    }
    if (question.options.length < MIN_OPTIONS) {
      return fail(
        "empty_options",
        `Error: each question needs ${MIN_OPTIONS}-${MAX_OPTIONS} options.`,
      );
    }
    ...
    for (const option of question.options) {
      ...
      if (RESERVED.has(option.label)) {
        return fail(
          "reserved_label",
          `Error: option label is reserved (${RESERVED_LABELS.join(", ")}).`,
        );
      }
      if (option.label.length > MAX_LABEL_LENGTH) {
        return fail(
          "label_too_long",
          `Error: option labels must be at most ${MAX_LABEL_LENGTH} characters.`,
        );
      }
