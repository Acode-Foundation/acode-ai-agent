...
export function buildQuestionnaireResponse(
  ...
  const segments = result.answers
    ...
    .filter(Boolean);
  if (!segments.length)
    return textResult(DECLINE_MESSAGE, { answers: result.answers, cancelled: true });
...
function formatSegment(answer: QuestionAnswer): string {
  ...
  if (answer.preview?.trim())
    parts.push(`selected preview: ${truncate(answer.preview.trim(), MAX_PREVIEW_CHARS)}`);
  return `${parts.join(". ")}.`;
}
